...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let usage = "usage: thermal_image <printer-host> <image-path> [double] [slow] [rotate] [density=N] [brightness=F] [contrast=F] [gamma=F] [equalize=0|1]";
    let host = args.next().ok_or(usage)?;
    let path = args.next().ok_or(usage)?;
    let flags: Vec<String> = args.collect();
    if let Some(bad) = flags.iter().find(|f| {
        !matches!(f.as_str(), "double" | "slow" | "rotate")
            && ![
                "density=",
                "brightness=",
                "contrast=",
